...
import { clearanceWorkflow } from '@/services/storage'; // Keep workflow utils
...
export default function DepartmentDashboard() {
  ...
  const [studentNames, setStudentNames] = useState<Record<string, string>>({}); // Cache student names
  ...
  useEffect(() => {
    if (user) {
      loadApprovals();
      // ... profile loading logic remains the same (localStorage is fine for simple profile pic preference)
    }
  }, [user]);
  ...
  const loadApprovals = async () => {
    if (!user) return;
    try {
      // Fetch approvals for this Role or Name from Backend
      const data = await apiService.getStaffApprovals(user.role, user.name);
      setApprovals(data);
      
      // Fetch student names for these approvals (Optimization: fetch all students once or batched)
      const allStudents = await apiService.getAllStudents();
      const nameMap: Record<string, string> = {};
      
      // Need to link Approval -> Request -> Student
      // Since Approval has requestId, we need to fetch requests or assume we can find student some other way.
      // Easiest for Dashboard: Get all Requests to map RequestID -> StudentID -> Name
      const allRequests = await apiService.getAllRequests();

      data.forEach((approval: ClearanceApproval) => {
         const req = allRequests.find((r: any) => r.id === approval.requestId);
         if (req) {
             const stud = allStudents.find((s: Student) => s.id === req.studentId);
             if (stud) nameMap[approval.id] = stud.name;
         }
      });
  ...
  const completionRate = approvals.length > 0 
    ...
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              ...
              <span className="text-primary font-semibold ml-1">
                {clearanceWorkflow.getDepartmentLabel(user.role) || "Subject Faculty"}
              </span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          {/* Profile Section (Kept visual logic same) */}
          <Card className="xl:col-span-1 shadow-md border-primary/10">
             {/* ... (Profile UI code same as before) ... */}
            ...
            <CardContent className="flex flex-col items-center">
              <div className="relative mb-6">
                <img 
                  ...
                  alt="Profile" 
                  className="h-32 w-32 rounded-full border-4 border-white shadow-lg object-cover bg-gray-200"
                />
                {isEditing && (
                  <>
                    <button 
                      ...
                      className="absolute bottom-1 right-1 p-2 bg-primary text-white rounded-full hover:bg-primary/90 transition-colors shadow-md"
                    >
                ...
                {isEditing ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-blue-600 font-semibold uppercase tracking-wider">
                        {clearanceWorkflow.getDepartmentLabel(user.role) || "Subject Specialist"}
                    </p>
                    <div className="mt-6 space-y-3 text-left border-t pt-4">
                      <div className="flex items-center text-sm text-gray-600">
                        <Mail className="mr-3 h-4 w-4" /> {profile.email}
                      </div>
                      <div className="flex items-center text-sm text-gray-600">
                        <ShieldCheck className="mr-3 h-4 w-4 text-green-600" /> Verified Authority
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div className="xl:col-span-2 space-y-6">
            <Card className="bg-primary/5 border-primary/20">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>Actions for pending student forms</CardDescription>
                </div>
                <ClipboardCheck className="h-8 w-8 text-primary/40" />
              </CardHeader>
              <CardContent className="flex gap-4">
                <Link to="/department/requests" className="flex-1">
                ...
                {pendingApprovals > 0 && (
                  <div className="flex-none px-6 bg-yellow-100 border border-yellow-200 rounded-lg flex items-center justify-center animate-pulse">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>History of student clearances you processed</CardDescription>
              </CardHeader>
              <CardContent>
                {approvals.length === 0 ? (
                  <div className="text-center py-10 bg-muted/20 rounded-lg border-2 border-dashed">
                    <p className="text-muted-foreground italic">No student activity recorded yet</p>
                  </div>
                ) : (
                      ...
                      <div key={approval.id} className="flex items-center justify-between p-4 border rounded-xl bg-white shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center text-primary font-bold">
                              {studentNames[approval.id]?.charAt(0) || 'S'}
                          </div>
                          <div>
                              <p className="font-bold text-sm text-foreground">{studentNames[approval.id] || 'Student'}</p>
