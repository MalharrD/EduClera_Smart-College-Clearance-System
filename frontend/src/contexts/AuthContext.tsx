...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  }, [user]); // Depend on user to avoid re-fetching if already set
  ...
  // 4. Register Function (Supabase + MongoDB)
  const register = async (userData: Partial<User>, studentData?: Partial<Student>) => {
    ...
    // B. Create User Profile in MongoDB
    const newUser: any = {
    ...
    // C. Create Student Profile if applicable
    if (userData.role === 'student' && studentData) {
      const newStudent = {
        ...studentData,
        userId: authData.user.id, // Link to Supabase ID
      };
