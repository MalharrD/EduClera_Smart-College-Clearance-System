...
import { fileURLToPath } from 'url'; // Added to support __dirname
import {
  makeTagger,
  ...
  plugins: [
...

    {
      ...
      configureServer(server) {
        ...
        // Fix: Added ': any' to payload to solve implicit any error
        server.ws.send = (payload: any) => {
          if (hmrEnabled) {
            return _send.call(server.ws, payload);
          } else {
            // Optional: Check if payload has a type before logging to avoid runtime errors if payload is null
